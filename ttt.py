	...
	def __huntElement(self, taskQueue, threadLock, taskResults):
		...
		while True:
			try:
				...
				# Get task sequnce
				taskSeq = QUEUESIZE - TaskThread.taskQueue.qsize()

				# Release the lock of share resource
				threadLock.release()

				LOGGER.info("Task started: " + task["Name"] + " in Thread(" + self.threadId + "), using " + BROWSER + ", " + str(taskSeq) + "/" + str(QUEUESIZE))
				...
				if "FirefoxHeadless" == BROWSER:
					...
					chromeOptions.add_argument('--disable-gpu')  # Temporarily needed if running on Windows
				...
				if 'browser' in dir() and browser != None:
					browser.quit()
					browser = None
...
try:
	...
	print("CONFIG_FILE: " + CONFIG_FILE + ", OUTPUT_FILE: " + OUTPUT_FILE)
	LOGGER.info("CONFIG_FILE: " + CONFIG_FILE + ", OUTPUT_FILE: " + OUTPUT_FILE)
